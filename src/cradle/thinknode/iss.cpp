...
parse_url_type(token_range_t& tokens)
{
    ...
    {
        return make_thinknode_type_info_with_boolean_type(thinknode_boolean_type());
    }
    if (type_code == "datetime")
    {
        return make_thinknode_type_info_with_datetime_type(thinknode_datetime_type());
    }
    if (type_code == "dynamic")
    {
        return make_thinknode_type_info_with_dynamic_type(thinknode_dynamic_type());
    }
    if (type_code == "enum")
    {
    ...
    {
        return make_thinknode_type_info_with_integer_type(thinknode_integer_type());
    }
    if (type_code == "map")
    {
    ...
    {
        return make_thinknode_type_info_with_optional_type(parse_url_type(tokens));
    }
    if (type_code == "reference")
    {
        return make_thinknode_type_info_with_reference_type(parse_url_type(tokens));
    }
    if (type_code == "string")
    {
        return make_thinknode_type_info_with_string_type(thinknode_string_type());
    }
    if (type_code == "structure")
    {
        ...
        for (size_t i = 0; i != n_fields; ++i)
        {
            ...
            structure_info.fields[name] = make_thinknode_structure_field_info("", none, schema);
        }
    ...
    if (type_code == "union")
    {
        ...
        for (size_t i = 0; i != n_members; ++i)
        {
            auto name = get_token(tokens);
            auto schema = parse_url_type(tokens);
            union_info.members[name] = make_thinknode_union_member_info("", schema);
        }
    ...
    catch (char const* message)
    {
        CRADLE_THROW(
            parsing_error() <<
                ...
                parsing_error_info(message) <<
                parsed_text_info(url_type));
    }
}
