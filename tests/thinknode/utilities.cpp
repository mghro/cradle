...
TEST_CASE("Thinknode type conversion", "[thinknode][utilities]")
{
    ...
    auto tn_struct_type =
        make_thinknode_type_info_with_structure_type(
            make_thinknode_structure_info(
                {
                    { "def", make_thinknode_structure_field_info("ijk", none, tn_array_type) },
                    { "abc", make_thinknode_structure_field_info("xyz", true, tn_blob_type) }
                }));
    auto struct_type =
    ...
    auto tn_union_type =
        make_thinknode_type_info_with_union_type(
            make_thinknode_union_info(
                {
                    { "def", make_thinknode_union_member_info("ijk", tn_array_type) },
                    { "abc", make_thinknode_union_member_info("xyz", tn_blob_type) },
                    { "ghi", make_thinknode_union_member_info("qrs", tn_string_type) }
                }));
    auto union_type =
    ...
    auto tn_optional_type = make_thinknode_type_info_with_optional_type(tn_map_type);
    auto optional_type = make_api_type_info_with_optional(map_type);
    REQUIRE(as_api_type(tn_optional_type) == optional_type);

    auto tn_enum_type =
        make_thinknode_type_info_with_enum_type(
            make_thinknode_enum_info(
                {
    ...
    auto tn_ref_type = make_thinknode_type_info_with_reference_type(tn_named_type);
