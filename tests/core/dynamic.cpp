...
TEST_CASE("dynamic deep_sizeof", "[core][dynamic]")
{
    ...
    REQUIRE(deep_sizeof(dynamic(array)) == sizeof(dynamic) + deep_sizeof(array));
...
{
    {
        INFO("Dynamic values containing empty maps can be treated as empty arrays.")
        REQUIRE(from_dynamic<std::vector<double>>(dynamic(dynamic_map())).empty());
        INFO("This doesn't work for non-empty maps.")
        REQUIRE_THROWS(
            from_dynamic<std::vector<double>>(
                dynamic(dynamic_map({ { dynamic(0.), dynamic(1.) } }))));
    }
    {
        INFO("Dynamic values containing empty arrays can be treated as empty maps.")
        REQUIRE((from_dynamic<std::map<double,double>>(dynamic(dynamic_array())).empty()));
        INFO("This doesn't work for non-empty arrays.")
        REQUIRE_THROWS((
            from_dynamic<std::map<double,double>>(
...
{
    auto type_dictionary =
        std::map<api_named_type_reference,api_type_info>(
            {
                {
    ...
    auto coerce_value =
        [&](api_type_info const& type, auto&& value)
        {
            return
                cradle::coerce_value(
    ...
    REQUIRE(coerce_value(integer_type, dynamic(integer(0))) == dynamic(integer(0)));
    // Test that doubles can be coerced to integers iff they're actually integers.
    REQUIRE(coerce_value(integer_type, dynamic(double(0))) == dynamic(integer(0)));
    ...
    REQUIRE(coerce_value(float_type, dynamic(double(0.5))) == dynamic(double(0.5)));
    // Test that integers can be coerced to doubles.
    REQUIRE(coerce_value(float_type, dynamic(integer(0))) == dynamic(double(0)));
    ...
    auto named_integer_type =
        make_api_type_info_with_named(
            make_api_named_type_reference("my_account", "my_app", "int"));
    auto named_float_type =
        make_api_type_info_with_named(
            make_api_named_type_reference("my_account", "my_app", "float"));
    REQUIRE(coerce_value(named_integer_type, dynamic(double(0))) == dynamic(integer(0)));
    ...
    REQUIRE(coerce_value(named_float_type, dynamic(integer(0))) == dynamic(double(0)));
    REQUIRE(coerce_value(named_float_type, dynamic(double(0.5))) == dynamic(double(0.5)));

    auto string_type = make_api_type_info_with_string(api_string_type());
    REQUIRE(coerce_value(string_type, dynamic(string("xyz"))) == dynamic(string("xyz")));
    ...
    auto test_datetime =
        ptime(
            ...
            boost::posix_time::time_duration(1,2,3));
    REQUIRE(coerce_value(datetime_type, dynamic(test_datetime)) == dynamic(test_datetime));
    REQUIRE_THROWS(coerce_value(datetime_type, dynamic(false)));

    auto integer_array_type =
        make_api_type_info_with_array(make_api_array_info(integer_type, none));
    auto test_integer_array =
        dynamic_array({ dynamic(integer(2)), dynamic(integer(0)), dynamic(integer(3)) });
    auto float_array_type =
        make_api_type_info_with_array(make_api_array_info(float_type, none));
    auto test_float_array =
        dynamic_array({ dynamic(double(2)), dynamic(double(0)), dynamic(double(3)) });
    auto test_boolean_array =
        dynamic_array({ dynamic(false), dynamic(true) });
    ...
    REQUIRE_THROWS(
        coerce_value(union_type, dynamic({ { "xyz", double(1) } })));
    REQUIRE_THROWS(
        coerce_value(union_type, dynamic({ { "abc", "xyz" } })));
    REQUIRE_THROWS(coerce_value(struct_type, dynamic(false)));
}
